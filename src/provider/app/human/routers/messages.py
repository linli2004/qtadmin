...
from app.human.schemas.messages import (
    ClaimOutboxResponse,
    DeadLetterItem,
    MailMessageRead,
    OutboxCountResponse,
...
def outbox_count(
    ...
    status: str | None = Query(None, description="Filter by send_status"),
):
    filter_statuses = [status] if status else ["pending", "sending"]
    count = (
        db.query(func.count(MailMessage.id))
        .filter(
            MailMessage.direction == "outbound",
            MailMessage.send_status.in_(filter_statuses),
        )
        .scalar()
    )
...
@router.get("/messages/outbox/dead", response_model=list[DeadLetterItem])
def list_dead_letters(db: Session = Depends(get_db)):
    items = (
        db.query(MailMessage)
        .filter(
            MailMessage.direction == "outbound",
            MailMessage.send_status == "failed",
            MailMessage.retry_count >= _OUTBOX_MAX_RETRIES,
        )
    ...
    return [
        DeadLetterItem(
            id=m.id, application_id=m.application_id, candidate_id=m.candidate_id,
            subject=m.subject, recipient_email=m.recipient_email,
            failure_reason=m.failure_reason, retry_count=m.retry_count or 0,
            last_retry_at=str(m.last_retry_at) if m.last_retry_at else None,
            created_at=str(m.created_at),
        )
        for m in items
    ]
...
@router.post("/messages/outbox/{message_id}/requeue", response_model=RequeueResponse)
def requeue_dead_letter(message_id: int, db: Session = Depends(get_db)):
    m = db.query(MailMessage).filter(MailMessage.id == message_id).first()
    if not m:
        raise HTTPException(404, "Message not found")
    if m.send_status != "failed" or (m.retry_count or 0) < _OUTBOX_MAX_RETRIES:
        raise HTTPException(400, "Message is not a dead letter (send_status must be 'failed' with retry_count >= 5)")
    ...
    db.commit()
    return RequeueResponse(id=m.id, send_status="pending", retry_count=0)
