...
from app.human.schemas.pending_queue import ConfirmRequest, ConfirmResponse, IgnoreRequest, QueueItemRead, QueueListResponse
from app.human.services.email_matcher import effective_email, find_active_application, find_candidate_by_email
from app.human.services.transition import transition_application
...
def confirm_queue_item(queue_id: int, body: ConfirmRequest, db: Session = Depends(get_db)):
    ...
    if not candidate:
        candidate = Candidate(email=effective_email_raw, real_name=effective_real_name)
        db.add(candidate)
        db.flush()
    ...
    if target_status and target_status != app.status.value:
        ...
        try:
            ...
            for stage in status_order[current_idx + 1 : target_idx + 1]:
                transition_application(app, TalentStatus(stage))
...
def queue_stats(db: Session = Depends(get_db)):
    ...
    for status in ("pending", "confirmed", "ignored"):
        ...
        unique = {effective_email(ext, snd) for ext, snd in rows if effective_email(ext, snd)}
        if unique:
