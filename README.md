...
- `packages/fastapi` **M4 完成** — 四个 ORM 模型 + Pydantic Schema + Alembic 迁移 + Normalizer 接口 + `CsvRowNormalizer` + `ManualNormalizer` + 标准化/分类/统计完整路由（121 tests）。参见 [`ROADMAP.md`](ROADMAP.md)。
- `demo/index.html` **全流程演示已上线** — 覆盖 M1–M4 全部 5 步产品流程：结构化录入 → 预览确认 → 自动预分类 → 批量审核 → 统计看板。
