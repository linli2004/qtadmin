...
- `packages/dart` 已发布 [`quanttide_finance`](https://pub.dev/packages/quanttide_finance) `^0.1.1`
- 当前 Dart 包提供的是 `Journal`、`JournalEntry`、`JournalEntryLine` 三个 freezed 不可变模型
- 这部分更接近“可选下游凭证层”，还不是完整的标准化主干
- `packages/fastapi`、`SourceRecord`、`RecordLink`、`NormalizedRecord`、`ClassificationResult`、统计 API 仍处于规划阶段
...
| `packages/fastapi` | 规划中 | 标准化、分类、统计的主服务层 |

## 近期计划

1. 建立 `SourceRecord`、`RecordLink`、`NormalizedRecord`、`ClassificationResult` 核心模型。
2. 提供标准化 API、分类 API、统计 API，而不是只围绕凭证 CRUD。
3. 允许 AI、规则、人工三种分类路径并存。
4. 把凭证生成改成可选下游能力，由 `NormalizedRecord` 映射而来。
5. 同步升级 Dart 模型，逐步对齐 `int id` 和“分”为单位的金额约定。
