...
| 里程碑 | 目标日期 | 关键交付物 | 通过标准 |
|:-------|:--------:|:-----------|:---------|
| **M0: 工程脚手架** | Day 0-3 | `pyproject.toml` + 依赖管理；Alembic 初始迁移；Docker Compose（开发用 SQLite / 生产用 PostgreSQL）；FastAPI CI 流水线（`fastapi-check.yml`）；pytest 框架 + 基础夹具（临时 SQLite 数据库） | `pytest` 可运行空测试套件通过；`docker compose up` 启动 API 返回健康检查 200 |
...
| 阶段 | 步骤 | 内容 |
|:-----|:-----|:-----|
| **M0** | 1 | 搭建项目骨架（`pyproject.toml`、Alembic、CI `fastapi-check.yml`） |
| | 2 | 实现健康检查端点 |
