...
from human.models.talent import ALLOWED_STATUSES_FOR_SUB_STAGE, STATUS_TRANSITIONS, Talent, TalentStatus
...
from human.schemas.talent import SubStageUpdate, TalentCreate, TalentRead, TalentTransition, TalentUpdate
from human.schemas.recruitment import HeadcountRead, RecruitmentRead
from human.services.headcount import get_headcount

router = APIRouter(prefix="/recruitments", tags=["human"])

@router.get("", response_model=list[RecruitmentRead])
def list_recruitments(skip: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=500), db: Session = Depends(get_db)):
...
def create_recruitment(db: Session = Depends(get_db)):
    r = Recruitment(); db.add(r); db.commit(); db.refresh(r); return r
...
def list_talents(recruitment_id: int, status: TalentStatus | None = None,
    ...
    if status: qb = qb.filter(Talent.status == status)
...
def get_talent(recruitment_id: int, talent_id: int, db: Session = Depends(get_db)):
    ...
    if not t: raise HTTPException(404, "Talent not found")
    return t
...
def create_talent(recruitment_id: int, data: TalentCreate, db: Session = Depends(get_db)):
    ...
    if not recruitment: raise HTTPException(404, "Recruitment not found")
    candidate = db.query(Candidate).filter(Candidate.email == data.email).first()
    if not candidate:
        candidate = Candidate(email=data.email, real_name=data.real_name); db.add(candidate); db.flush()
    app = Application(candidate_id=candidate.id, recruitment_id=recruitment_id, source="manual_debug"); db.add(app); db.flush()
...
def transition_talent(recruitment_id: int, talent_id: int, data: TalentTransition, db: Session = Depends(get_db)):
    t = db.query(Talent).filter(Talent.id == talent_id, Talent.recruitment_id == recruitment_id).first()
    if not t: raise HTTPException(404, "Talent not found")
    if data.status not in STATUS_TRANSITIONS.get(t.status, []):
        raise HTTPException(400, f"Cannot transition from {t.status.value} to {data.status.value}")
    candidate = db.query(Candidate).filter(Candidate.email == t.email).first()
    if candidate:
        app = db.query(Application).filter(Application.candidate_id == candidate.id,
            Application.recruitment_id == recruitment_id).order_by(Application.created_at.desc()).first()
        if app:
            app.status = data.status
            if data.status != t.status: app.sub_stage = None
            if data.sub_stage is not None and data.status in ALLOWED_STATUSES_FOR_SUB_STAGE:
                app.sub_stage = data.sub_stage
            t.status = app.status; t.sub_stage = app.sub_stage; t.stage_results = app.stage_results
    else:
...
def set_talent_sub_stage(recruitment_id: int, talent_id: int, data: SubStageUpdate, db: Session = Depends(get_db)):
    ...
    t.sub_stage = data.sub_stage; db.commit(); db.refresh(t); return t
